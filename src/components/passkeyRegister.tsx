...
export function PasskeyRegister() {
	async function passkeyRegister(e) {
		...
		const jwt = (await fetch("/api/passkey/register/start").then(res => res.json())).jwt;
		...
		const registrationSuccess = await fetch("/api/passkey/register/finish", {
			method: "POST",
			headers: {
	...
	return (
		<div id="passkeyRegister">
			<form>
				<button className="bg-blue-600 hover:bg-blue-500 text-white font-semibold p-2 mt-3 rounded w-[100%]" onClick={handleClick}>Create Passkey</button>
			</form>
		</div>
	);
}
